...
public class SoldierAttack extends  Weapon{
    ...
    public float width;
    public float height;
    private Weapon.UsedState usedState;
    private long throwStartTime;
    private long deathStart;
    private float throwTimeKeeper;
    private float deathTime;
    public SoldierAttack(Vector2 position) {
        this.position = position;
        region = Assets.ourInstance.bulletPulseAssets.pulse0;
    ...
    public void update(float delta, DelayedRemovalArray<Enemy> enemies) {
        ...
        if(usedState == UsedState.AVAILABLE) {
            region = (TextureRegion) Assets.ourInstance.bulletPulseAssets.slashAnimation.getKeyFrame(throwTimeKeeper);
        } else {
