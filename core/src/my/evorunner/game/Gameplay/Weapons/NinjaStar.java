...
public class NinjaStar extends Weapon {
    ...
    public NinjaStar(Vector2 position) {
        ...
        region = Assets.ourInstance.ninjaStarAssets.NINJA_STAR_90;
    ...
    public void update(float delta, DelayedRemovalArray<Enemy> enemies) {
        ...
        for (Enemy enemy : enemies) {
            ...
            if (enemyBound.overlaps(bounds) && usedState == UsedState.AVAILABLE) {
                enemy.hitByWeapon();
        ...
        if(usedState == UsedState.AVAILABLE) {
            region = (TextureRegion) Assets.ourInstance.ninjaStarAssets.ninjaStarAnimation.getKeyFrame(throwTimeKeeper);
        } else {
